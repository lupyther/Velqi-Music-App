...
Future<void> main() async {
  ...
  await initHive();
  _setAppInitPrefs();
  // Start the embedded Python backend (yt-dlp + ytmusicapi) early so it is warm
  // by the time the user searches/plays. Fire-and-forget: the UI launches now;
  // callers await BackendService.ensureReady() lazily.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GetMaterialApp(
        ...
        locale:
            Locale(Hive.box("AppPrefs").get('currentAppLanguageCode') ?? "es"),
...
void _setAppInitPrefs() {
  ...
  if (appPrefs.isEmpty) {
    appPrefs.putAll({
      ...
      'currentAppLanguageCode': 'es', // Velqi: Spanish by default
