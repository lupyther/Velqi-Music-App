...
class PlayerController extends GetxController
  ...
  /// Fully stops playback, clears queue, and resets the player state.
  /// Use this when the user wants to dismiss the current song entirely.
  Future<void> stopPlayback() async {
    // Hide the UI instantly first so the X feels immediate; stop audio after.
    isPlayerpanelTopVisible.value = false;
    playerPanelController.close();
    playerPanelMinHeight.value = 0.0;
    // Reset initFlag so Home widget won't restore minHeight on rebuild
    initFlagForPlayer = true;
    currentSong.value = null;
    progressBarStatus.value = ProgressBarState(
        buffered: Duration.zero, current: Duration.zero, total: Duration.zero);
    await _audioHandler.customAction("stopPlayback");
  }
