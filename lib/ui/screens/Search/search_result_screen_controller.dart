...
class SearchResultScreenController extends GetxController
  ...
  Future<void> onDestinationSelected(int value,
    ...
    if (value > 0 &&
      ...
      if (tabName == 'Songs' || tabName == 'Videos') {
        _prefetchSongs((x[tabName] as List).take(3));
      }
  ...
  Future<void> _getInitSearchResult() async {
    ...
    if (args != null) {
      ...
      if (resultContent.containsKey("Songs")) {
        final songs = resultContent["Songs"] as List;
        _prefetchSongs(songs.take(3));
      }
      isResultContentFetced.value = true;
    }
  }

  void _prefetchSongs(Iterable<dynamic> items) {
    try {
      final audioHandler = Get.find<AudioHandler>();
      for (final item in items) {
        if (item != null && item['id'] != null) {
          audioHandler.customAction('prefetchSong', {'songId': item['id'].toString()});
        }
      }
    } catch (e) {
      printINFO("Prefetch error: $e");
    }
  }
