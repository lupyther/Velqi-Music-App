...
class SearchScreenController extends GetxController with ProcessLink {
  ...
  final isBackendReady = false.obs;
  ...
  void _waitBackend() async {
    if (BackendService.instance.isReady) {
      isBackendReady.value = true;
      return;
    }
    // Esperar hasta 45s a que el backend esté listo
    final ok = await BackendService.instance
        .ensureReady(timeout: const Duration(seconds: 45));
    isBackendReady.value = ok;
  }
