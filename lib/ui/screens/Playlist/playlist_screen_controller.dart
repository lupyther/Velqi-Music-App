...
import 'package:velqi/models/thumbnail.dart';
...
import 'package:velqi/ui/widgets/snackbar.dart';
...
class PlaylistScreenController extends PlaylistAlbumScreenControllerBase
    ...
    try {
      ...
      if (await checkIfAddedToLibrary(playlistId)) {
        ...
        if (songsBox.values.isEmpty) {
          _fetchSongOnline(playlistId, isIdOnly).then((value) {
            updateSongsIntoDb();
          });
        } else {
        ...
        _fetchSongOnline(playlistId, isIdOnly);
      }
      isContentFetched.value = true;
    } catch (e) {
  ...
  void syncPlaylistSongs() {
    _fetchSongOnline(playlist.value.playlistId, false).then((value) {
      updateSongsIntoDb();
      isContentFetched.value = true;
    });
  }
  ...
  String _generateCsvContent() {
    ...
    for (final song in songList) {
      ...
      final playlistBrowseId = (!playlist.value.isCloudPlaylist)
          ? ''
  ...
  String _getLocationMessage(String path) {
    if (Platform.isAndroid) {
      return "Downloads/Velqi";
    } else if (Platform.isIOS) {
      return "Files App > Velqi";
    } else if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) {
      return "Downloads/Velqi";
    } else {
